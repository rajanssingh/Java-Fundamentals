...
public class Queries {
    public static void main(String[] args) {
        ...
        employeeList.add(new Employee(177, "Manu Sharma", 35, "Male", "Account And Finance", 2010, 59000.0));
        ...
        employeeList.add(new Employee(244, "Nicolus Den", 24, "Male", "Sales And Marketing", 2017, 100700.5));
        ...
        // 16. Highest employee in every bracket ( < 60k and > 60k)
        Map<Boolean, Optional<Employee>> collect = employeeList.stream().collect(Collectors.groupingBy(e -> e.salary < 60000, Collectors.maxBy((e1, e2) -> (int) (e1.getSalary() - e2.getSalary()))));
        System.out.println("16-----------------------");
        collect.entrySet().stream().forEach(System.out::println);

        // 16. total salary expenditure in every bracket ( < 60k and > 60k)
        Map<Boolean, Double> collect1 = employeeList.stream().collect(Collectors.groupingBy(e -> e.salary < 60000, Collectors.summingDouble(Employee::getSalary)));
        Map<Boolean, Double> collect2 = employeeList.stream().collect(Collectors.groupingBy(e -> e.salary < 60000, Collectors.reducing(0.0, Employee::getSalary, Double::sum)));
        System.out.println("16-----------------------");
